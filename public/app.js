// =====================================================
// Total Battle tracker — v5
// Same features as v4 but with loud error logging
// and simplified post-login flow to avoid silent hangs.
// =====================================================
...
function fatalError(msg) {
  console.error("[TB ERROR]", msg);
  const banner = document.createElement("div");
  banner.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:9999;background:#8b2e2e;color:#fff;padding:12px 20px;font-family:monospace;font-size:13px;border-bottom:2px solid #b04545;";
  banner.textContent = "ERROR: " + msg;
  document.body.appendChild(banner);
}

window.addEventListener("error", (e) => fatalError("JS error: " + e.message));
window.addEventListener("unhandledrejection", (e) => fatalError("Promise rejected: " + (e.reason?.message || e.reason)));
...
console.log("[TB] booting with URL:", SUPABASE_URL);
...
async function handleAuthSubmit() {
  ...
  try {
    if (state.signupMode) {
      ...
      if (!username || username.length < 2) { showAuthMsg("Username must be at least 2 characters."); return; }
      if (!code) { showAuthMsg("Group code required."); return; }

      console.log("[TB] verifying signup code...");
      ...
      console.log("[TB] signing in...");
      showAuthMsg("Signing in…", "info");
      const { data, error } = await sb.auth.signInWithPassword({ email, password });
      if (error) { showAuthMsg(error.message); return; }
      console.log("[TB] sign in OK, user id:", data.user?.id);
      // Don't rely on onAuthStateChange — load app directly
      await loadProfileAndApp();
    }
  } catch (err) {
    console.error("[TB] auth error:", err);
    showAuthMsg("Unexpected error: " + err.message);
  } finally {
    btn.disabled = false;
  }
}
...
async function boot() {
  ...
  const cBtn = $("#save-captains-btn"), cBtn2 = $("#save-captains-btn-2");
  if (cBtn) cBtn.addEventListener("click", saveCaptainsForm);
  if (cBtn2) cBtn2.addEventListener("click", saveCaptainsForm);
  const aBtn = $("#save-artifacts-btn"), aBtn2 = $("#save-artifacts-btn-2");
  if (aBtn) aBtn.addEventListener("click", saveArtifactsForm);
  if (aBtn2) aBtn2.addEventListener("click", saveArtifactsForm);

  setAuthMode("signin");

  console.log("[TB] checking existing session...");
  const { data, error } = await sb.auth.getSession();
  if (error) { fatalError("Session check failed: " + error.message); return; }
  if (data.session) {
    console.log("[TB] existing session found, loading app");
    await loadProfileAndApp();
  } else {
    console.log("[TB] no session, showing auth");
    showAuthScreen();
  }

  console.log("[TB] boot complete");
}

async function loadProfileAndApp() {
  console.log("[TB] loadProfileAndApp start");
  const { data: { user }, error: uErr } = await sb.auth.getUser();
  if (uErr || !user) {
    fatalError("Could not get user: " + (uErr?.message || "no user"));
    showAuthScreen();
    return;
  }
  state.user = user;
  console.log("[TB] user:", user.id);

  // Fetch profile — single attempt, no waiting loop (was hanging before)
  console.log("[TB] fetching profile...");
  const { data: profile, error: pErr } = await sb.from("profiles").select("*").eq("id", user.id).maybeSingle();
  if (pErr) {
    fatalError("Profile fetch failed: " + pErr.message);
    return;
  }
  if (!profile) {
    fatalError("No profile row for user " + user.id + ". Sign up may not have completed. Check Supabase profiles table.");
    return;
  }
  state.profile = profile;
  console.log("[TB] profile:", profile.username, "admin:", profile.is_admin);
  ...
  console.log("[TB] loading profiles + rosters...");
  await Promise.all([loadAllProfiles(), loadRosters()]);

  console.log("[TB] rendering active tab:", state.activeTab);
  await refreshActiveTab();
  console.log("[TB] app loaded successfully");
}

async function loadAllProfiles() {
  const { data, error } = await sb.from("profiles").select("id, username, is_admin").order("username");
  if (error) { fatalError("Load profiles failed: " + error.message); return; }
  state.profiles = data || [];
...
async function loadRosters() {
  ...
  if (c.error) { fatalError("Captain roster load failed: " + c.error.message); return; }
  if (a.error) { fatalError("Artifact roster load failed: " + a.error.message); return; }
  state.captainRoster = c.data || [];
  state.artifactRoster = a.data || [];
  console.log("[TB] roster loaded:", state.captainRoster.length, "captains,", state.artifactRoster.length, "artifacts");
}
...
async function getLatestSnapshots(userId) {
  ...
  for (const s of cap.data || []) if (!latestCaptain.has(s.captain_id)) latestCaptain.set(s.captain_id, s);
  const latestArtifact = new Map();
  for (const s of art.data || []) if (!latestArtifact.has(s.artifact_id)) latestArtifact.set(s.artifact_id, s);
  const latestHero = (hero.data || [])[0] || null;

  const captainHistory = new Map();
...
async function renderMyRoster() {
  ...
  heroEl.innerHTML = data.latestHero
    ? `<div class="card-grid">${renderItemCard({ name: "Hero", latest: data.latestHero, history: data.heroHistory, kind: "hero" })}</div>`
    : `<div class="empty-state">No hero recorded yet.${isMine ? " Use the Update Captains tab." : ""}</div>`;

  const capList = $("#captains-list");
  capList.innerHTML = data.latestCaptain.size === 0
    ? `<div class="empty-state">No captains recorded yet.${isMine ? " Use the Update Captains tab." : ""}</div>`
    : state.captainRoster.filter(r => data.latestCaptain.has(r.id))
        .map(r => renderItemCard({ name: r.name, latest: data.latestCaptain.get(r.id), history: data.captainHistory.get(r.id) || [], kind: "captain" })).join("");

  const artList = $("#artifacts-list");
  artList.innerHTML = data.latestArtifact.size === 0
    ? `<div class="empty-state">No artifacts recorded yet.${isMine ? " Use the Update Artifacts tab." : ""}</div>`
    : state.artifactRoster.filter(r => data.latestArtifact.has(r.id))
        .map(r => renderItemCard({ name: r.name, latest: data.latestArtifact.get(r.id), history: data.artifactHistory.get(r.id) || [], kind: "artifact" })).join("");
}

function renderItemCard({ name, latest, history, kind }) {
  const prev = history[1];
  const levelDelta = (prev && latest.level != null && prev.level != null) ? latest.level - prev.level : 0;
  const starsDelta = (prev && latest.stars != null && prev.stars != null) ? latest.stars - prev.stars : 0;
...
function renderSparkline(history) {
  ...
  const w = 260, h = 36, stepX = w / (values.length - 1);
...
async function renderCaptainsForm() {
  ...
  const heroSection = `
    <div class="survey-section">
      ...
      <div class="survey-header"><span></span><span class="col-r">Level</span><span class="col-r">Stars</span></div>
...
function validateInput(input) {
  ...
  if (!Number.isInteger(n) || n < min || n > max) { input.classList.add("invalid"); return false; }
...
async function processSave(rows, msgElId, refreshFn) {
  ...
  for (const row of rows) {
    ...
    if (kind === "hero") inserts.hero.push({ user_id: state.user.id, level, stars });
    else if (kind === "captain") inserts.captain.push({ user_id: state.user.id, captain_id: Number(id), level, stars });
    else if (kind === "artifact") inserts.artifact.push({ user_id: state.user.id, artifact_id: Number(id), level, stars, petals });
  }
...
async function renderCompare() {
  ...
  for (const s of heroRes.data || []) if (!userHero.has(s.user_id)) userHero.set(s.user_id, s);
  ...
  container.innerHTML = `
    ...
    <div style="overflow-x:auto;">
      <table class="compare-table"><thead><tr><th>Player</th>${captainHeader}</tr></thead><tbody>${captainRows}</tbody></table>
    </div>
    <h3 class="section-h">Artifacts</h3>
    <div style="overflow-x:auto;">
      <table class="compare-table"><thead><tr><th>Player</th>${artifactHeader}</tr></thead><tbody>${artifactRows}</tbody></table>
    </div>`;
}
...
async function openAdminModal() {
  ...
  $("#modal").innerHTML = `
    ...
    <div class="field"><label>Members</label><div id="m-members" style="max-height:200px;overflow-y:auto;"></div></div>
    <div class="modal-actions"><button class="primary-btn" id="m-close">Close</button></div>`;
  show($("#modal-backdrop"));
  ...
  $("#m-save-code").addEventListener("click", async () => {
    ...
    const { error } = await sb.from("group_settings").update({ signup_code: newCode, updated_at: new Date().toISOString() }).eq("id", 1);
  ...
  membersEl.querySelectorAll("[data-toggle-admin]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    if (error) return alert(error.message);
    await loadAllProfiles(); openAdminModal();
  }));
  membersEl.querySelectorAll("[data-remove]").forEach(btn => btn.addEventListener("click", async () => {
...
if (backdrop) backdrop.addEventListener("click", e => { if (e.target.id === "modal-backdrop") hide(backdrop); });

boot().catch(err => fatalError("Boot failed: " + err.message));
